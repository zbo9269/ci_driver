...
# PLATFORM: Linux only
#
# PURPOSE: this script make easy for load and reload net module
#
# REV LIST  : 1.1.D
# DATE      : 20140320
# BY        : zhangys
...
function load_device ()
{
    ...
    if ${INSMOD} ${devpath}; then
        ...
        if [ "" == "${MAJOR}" ];then
            echo "can't get MAJOR from /proc/devices"
            return 1
        fi

        count=0
        for major in ${MAJOR};do
            count=$((count + 1))
        done
        if [ ${count} -gt 1 ];then
            echo "too many device found in /proc/devices"
            return 1
        fi
