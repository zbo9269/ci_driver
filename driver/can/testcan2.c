...
static char send_buffer[CAN_SEND_SIZE]={0x88,0x01,0x05,0x00,0x00,0x11,0x11,0x11,0x11,0x00,0x00,0x00,0x00};
...
int main()
{
	...
	/*for (i = 13; i > 0;i--)
	{
		send_buffer[13-i] = i & 0xff;
	}*/

	fd = open("/dev/can2",O_RDWR);

	if(-1 == fd)
	{
		perror("can't open can2");
	}
	else
	{
		printf("can2 has opened,fd is:%d\n",fd);
	}
